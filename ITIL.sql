...
SELECT 
    BASE.*,
    QUEUE.QUEUENAME          QUEUE_NAME, --工作组
    STATUS.STATUSNAME        STATUS_NAME, -- 状态
    USERT.CINAME             OWNER_NAME, --指派给 
    ORG.NAME                 SITE_NAME, --地点
    PRID.PRIORITYNAME        PRIORITY_NAME --优先级
FROM
(
    SELECT
...
SELECT
    BASE.*,
    CT.NAME              WORKLOG_TYPE_NAME, --日志类型 
...
(
    SELECT 
        ...
        C.CREATEDTIME   CREATED_TIME, --创建时间
        C.TS_STARTTIME  TS_STARTTIME, --开始时间
        ...
        C.WORKLOGTYPEID WORKLOG_TYPE_ID --日志类型 ID Y
    FROM
...
LEFT JOIN WorkLogTypeDefinition CT ON CT.WORKLOGTYPEID = BASE.WORKLOG_TYPE_ID;


CREATE VIEW V_TECHNICIAN AS
SELECT 
        CI.CIID, 
        CI.CINAME, 
        U.USER_ID,
        U.FIRST_NAME 
FROM CI CI, AaaUser U 
WHERE CI.CIID IN (SELECT * FROM Technician) 
AND CI.CINAME = U.FIRST_NAME;
